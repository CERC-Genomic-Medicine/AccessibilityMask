...
argparser.add_argument('-mdp', '--mean-dp', metavar = 'number', dest = 'in_mean_depth', type = int, required = True, help = 'Threshold for mean depth to only keep regions with resonable mean depth.')
...
def load_coverage_data(mean_depth, path_read = '/path/to/depth/data', path_write='/path/to/bed/file',  min_depth = "10X", min_percent = 1):
    ...
    column_name = f'PCT_INDV_OVER_{min_depth}_X'
...
if __name__ == "__main__":
    ...
    load_coverage_data(args.in_mean_depth, path_read = depth_file, path_write = out_file_path, min_depth = min_depth, min_percent = min_percent)
    exit()
